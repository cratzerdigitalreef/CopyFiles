...
def loci_imeiFromHexa(sIMEIHex, bDes=False, bLuhnCheck=True):
    ...
    sLog = sLog + "\nType of Identity (ETSI 124.008): " + imei[0:1] + " = " + bytes_fBinaryByHex(
        imei[0:1]) + " - bits [321] = " + sBits + " - Type = "
    sIMEIType = "reserved"
    if sBits == "001":
        sIMEIType = "IMSI"
    if sBits == "010":
    ...
    sLog = sLog + "\nIMEI (International Mobile Equipment Identity): " + str(imei[1:]) + " (Length: " + str(
        len(imei[1:])) + " digits)"
    ...
    sLog = sLog + "\nIMEI to be compared to the one from Mobile with '*#06#': " + str(sIMEIt) + " - length: " + str(
        len(sIMEIt)) + " digits"
...
def loci_lociFromHexa(sLOCIHex, bDes=False):
    ...
    sLog = sLog + "\nLOCI prepared: " + str(loci) + " (Length: " + str(len(loci)) + " hexadecimal characters - " + str(
        len(loci) / 2) + " bytes)"
    ...
    """
    REMOVED FOR NEW CALCULATION USED BY loci_cell_id_from_hexa FUNCTION
    """
    # if subcellid!="":
    #    sF="F"
    ...
    cell = loci_cell_id_from_hexa(cellid + subcellid)
    ...
    sLog = sLog + "\neNodeB calculated by Cell ID (Cell ID " + str(nCellID) + " / 256): 0x" + str(
        bytes_NroToHexa(neNodeB)) + " - Decimal: " + str(neNodeB)
...
def loci_lociToHexa(sMCCNro, sMNCNro, sAreaNro, sCellNro, bDes=False):
    ...
    if len(sLoci) % 2 != 0:
        sLog = sLog + "\nWARNING LOCI length is not correct, it must be bytes, with even characters. Length: " + str(
            len(sLoci)) + " characters."
...
# loci_cell_id_from_hexa---------------------------------------------------------------------------------------------------------------------------------------------------------
# Cell id hex examples:
# 0049A03F
# 0049
def loci_cell_id_from_hexa(cell_id_hex: str, description_mode: bool=False):
    """Processes a raw hexadecimal LOCI cell ID into a decimal format.

    The function cleans the input, handles sub-cell ID extensions (stripping
    trailing 'F's and ignoring all-zero extensions), and ensures the
    final hex string has an even number of characters.

    Args:
        cell_id_hex (str): The raw hexadecimal cell ID string
            (e.g., "0049A03F").
        description_mode (bool, optional): If True, returns a descriptive
            log. Defaults to False.

    Returns:
        str: The processed cell ID string (if description_mode=False)
             or a processing log string (if description_mode=True).
    """
    cell_id_hex = str_SpacesOut(cell_id_hex).upper()
    cell_id = cell_id_hex[0:4]
    log = "Cell ID to be processed: 0x" + str_AddSpaceHexa(cell_id)

    if len(cell_id_hex) > 4:
        sub_cell_id = cell_id_hex[4:]

        if sub_cell_id.endswith("F"):
            sub_cell_id = sub_cell_id[:-1]

        # Check if sub_cell_id is not empty AND not all zeros
        if sub_cell_id and sub_cell_id != ("0" * len(sub_cell_id)):
            cell_id_processed = cell_id + sub_cell_id
        else:
            cell_id_processed = cell_id
    else:
        cell_id_processed = cell_id

    if (len(cell_id_processed) % 2) != 0:
       cell_id_processed = "0" + cell_id_processed

    log += f"\nCell ID processed: {cell_id_processed} - Cell Id Calculated (decimal): {str(int(cell_id_processed, 16))}"

    return log if description_mode else cell_id_processed
# --------------------------------------------------------------------------------------------------------------------------------------------------------
